...
class NodeRunner:
    ...
    def run(self):
        ...
        address_hex = to_normalized_address(address) if address else None
        address_hex, privatekey_bin = prompt_account(
            address_hex,
            self._options['keystore_path'],
            self._options['password_file'],
        )
        address = to_canonical_address(address_hex)
        self._options['address'] = address
        self._options['address_hex'] = address_hex
        self._options['privatekey_bin'] = privatekey_bin
        net_id = self._options['network_id']
        ios_date = datetime.now().isoformat()

        configure_logging(
            self._options['log_config'],
            log_json=self._options['log_json'],
            log_file=self._options['log_file'],
            debug_log_file_name=f'raiden-debug-node_{pex(address)}-net_{net_id}-{ios_date}.log',
...
>>>>>>> Moved prompt_account out of run_app to avoid NoneType address
